...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent)
{
    ...
    phbxLayout0 = new  QBoxLayout(QBoxLayout::LeftToRight);
        ...
        qvblf = new QBoxLayout(QBoxLayout::TopToBottom);
            titlef = new QLabel("Results");
            imgf = new QLabel();
            titlef->setStyleSheet("font-size:10pt; font-weight:600; color:#000000;");
            titlef->setAlignment(Qt::AlignCenter);
            imgf->setStyleSheet("border: 1px solid gray");
            //img0->setMaximumSize(QSize(522,462));
            imgf->setAlignment(Qt::AlignCenter);
            imgf->setMinimumSize(QSize(522,462));
            qvblf->addWidget(titlef);
            qvblf->addWidget(imgf);
        phbxLayout0->addLayout(qvblf);
...
void MainWindow::visualizeImage(int id, QImage img, std::string title)
{
    ...
    case PROC5:
        ...
        break;
    case PROC6:
        imgf->setPixmap(image.scaled(imgf->size(), Qt::KeepAspectRatio));
        if(title == "") title = "RESULTADO FINAL";
        titlef->setText(QString::fromStdString(title));
        break;
    }
}
